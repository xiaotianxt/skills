...
|---|---|---|
| **Pi** | System prompt mentions "pi" OR `PI_*` env vars present; sessions in `~/.pi/agent/sessions/` | [pi.md](pi.md) |
...
**Detection priority:**
1. Check your system prompt / identity (e.g. "pi" vs "Codex" vs "opencode") or environment variables (`PI_*`).
2. If ambiguous, check which session store exists on disk.

Once detected, read the corresponding file:

- For **Pi**: open `pi.md` in the same directory as this file.
...
## Common safety rules (all agents)
